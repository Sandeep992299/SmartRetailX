...
3. **Dead Letter Queues (DLQ)**: In the asynchronous event pipeline (Order Service ➔ Kafka ➔ Payment Service), if a message fails processing after maximum retries, the consumer forwards it to a designated DLQ topic (e.g., `payment-failures-dlq`). This prevents head-of-line blocking on Kafka partitions while preserving the failed event for manual auditing or re-processing.
